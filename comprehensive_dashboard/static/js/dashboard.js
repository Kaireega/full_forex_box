...
async function loadChartData() {
    ...
    try {
        ...
        if (data && data.candles && data.candles.length > 0) {
            ...
            console.log('Creating chart with candles:', data.candles);
            createCandlestickChart(data.candles);
            updateChartInfo(data.candles);
        } else {
            console.error('❌ No candle data received from OANDA API');
...
function createCandlestickChart(candles) {
    console.log('Creating Plotly candlestick chart with', candles.length, 'candles');
    
    // Transform data for Plotly
    const dates = candles.map(candle => new Date(candle.x));
    const opens = candles.map(candle => parseFloat(candle.o));
    const highs = candles.map(candle => parseFloat(candle.h));
    const lows = candles.map(candle => parseFloat(candle.l));
    const closes = candles.map(candle => parseFloat(candle.c));
    ...
    console.log('✅ Plotly candlestick chart created successfully');
    showNotification('Chart updated successfully', 'success');
}

// Create line chart using Plotly (fallback)
function createLineChart(candles) {
    console.log('Creating Plotly line chart with', candles.length, 'candles');
    ...
    currentChart = chartContainer;
    console.log('✅ Plotly line chart created successfully');
    showNotification('Line chart created successfully', 'success');
}

// Update chart info display
function updateChartInfo(candles) {
    if (!candles || candles.length === 0) {
        console.warn('No candles data for chart info update');
        return;
    }
        ...
        chartInfo.innerHTML = `
            <div class="chart-info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">${parseFloat(latest.o).toFixed(5)}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">High:</span>
                    <span class="info-value">${parseFloat(latest.h).toFixed(5)}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Low:</span>
                    <span class="info-value">${parseFloat(latest.l).toFixed(5)}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Close:</span>
                    <span class="info-value">${currentPrice.toFixed(5)}</span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">${candles.length}</span>
                </div>
            </div>
        `;
    }
    
    console.log('✅ Chart info updated');
}
